...
export default function ViewProjectDocumentsTable({
  ...
  const sortedDocuments = [...documentsData].sort((a, b) => {
    ...
    if (sortBy === "createdTime") {
      ...
      comparison = (a.projectData.projectName || "").localeCompare(
        b.projectData.projectName || ""
      );
    } else if (sortBy === "documentName") {
      comparison = (a.documentData.name || "").localeCompare(
        b.documentData.name || ""
      );
    }
            ...
            {displayedDocuments.length > 0 ? (
              displayedDocuments.map((doc, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>{doc.projectData.projectName}</TableCell>
                  <TableCell>
                    {formatTimestampToDate(doc.documentData.createdTime)}
                  </TableCell>
                  <TableCell>Proje Belgesi</TableCell>
                  <TableCell>
                    <IconButton
