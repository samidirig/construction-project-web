...
import ViewCompanyDocuments from "../components/ViewModals/ViewCompanyDocuments";
import ViewCompanyDetails from "../components/ViewModals/ViewCompanyDetails";
...
export default function VisorCompanies() {
  const windowScreenWidth = useWindowSizeWidth();
  const [companiesData, setCompaniesData] = useState(companiesMap);
  ...
  //       try {
  //         const visor = await getVisorCompanyByManagerId();
  //         setVisorData(visor || []);
  //       } catch (error) {
  //         console.error("Error fetching visor:", error);
  //       } finally {
  //         setLoading(false);
  //       }
  //     }
  ...
  //     async function fetchData() {
  //       try {
  //         const companies = await getVisorCompanies();
  ...
  return (
    <div>
      ...
      >
        ...
        ) : companiesMap && companiesMap.length > 0 ? (
          companiesMap.map((company, index) => (
            <Card
              key={index}
              sx={{
                ...companyCardContent,
